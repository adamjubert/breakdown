...
class Game {
  ...
  addBricks() {
    let canvasWidth = this.width / 2 - 10;
  ...
  update() {
    ...
    this.pongBall.update(this.player.aiPaddle);
  ...
  update(paddle) {
    ...
    if ( this.ballHitsPaddle(paddle) ) {
        ...
        if (paddle.id === "breakOutPaddle") {
          ...
          this.x_speed = this.startingSpeed * Math.cos(bounceAngle);
          this.y_speed = this.startingSpeed * -Math.sin(bounceAngle);
